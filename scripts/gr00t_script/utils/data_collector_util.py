...
class DataCollector:
    def __init__(self, output_video_dir, output_data_dir, fps, video_format='mp4v'):
        ...
        self.global_frame_index_offset = 0
        self.task_index = 0
    ...
    def save_episode(self, episode_data):
        ...
        data = {
            "observation.state": pd.Series(obs_list_buffer, dtype=np.float32),
            "action": pd.Series(action_list_buffer, dtype=np.float32),
            "timestamp": timestamps,
            "frame_index": list(range(num_frames_in_episode)),
            "episode_index": [self.episode_index] * num_frames_in_episode,
