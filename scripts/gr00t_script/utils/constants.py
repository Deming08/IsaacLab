...
if g1_hand_type == "trihand":
    ...
    HAND_JOINT_POSITIONS = {
        ...
        "R_thumb_proximal_yaw_joint":   {"open": 1.18, "closed": 1.18},
...
if g1_hand_type == "trihand":
    ...
    # 1. Open the drawer handle (Relative to drawer's origin and orientation)
    PRE_APPROACH_OFFSET_POS     = np.array([-0.24350000, -0.04801383, 0.08380000])
    APPROACH_OFFSET_POS         = np.array([-0.18850000, -0.06160968, 0.08380000])  # Grasp the right side of the handle (y: -0.0357 -> -0.4357)
    PULL_OFFSET_POS             = np.array([-0.38231688, -0.06160968, 0.08380000])
    PRE_APPROACH_OFFSET_QUAT    = np.array([93, 41, 1.5])  # degrees, relative to drawer's orientation
    # 2. Pick-and-place the mug
    # Absolute POS and QUAT for avoiding singularity during transition
    MUG_APPROACH_POS            = np.array([0.20, 0.27, 0.90])  
    MUG_APPROACH_QUAT           = np.array([90, 0, -60])
    ...
    PRE_MAT_PLACE_POS           = np.array([-0.11000000, 0.10500000, 0.19500000])  #
    MAT_PLACE_POS               = np.array([-0.11000000, 0.10500000, 0.16500000])  # Enlarge the height (0.16 -> 0.165) to avoid collision with the mat
    MAT_PLACE_ABS_QUAT          = np.array([84.54269505, 17.98494010, -31.31594710])    # Absolute orientation

    DRAWER_PUSH_DIRECTION_LOCAL = np.array([0.200, 0, 0.02])   # Rise the hand 0.02 in z to keep holding the handle while pushing
    # 3. pick the bottle, pour it, and place it back
    BOTTLE_GRASP_POS            = np.array([-0.19999976, -0.06999982, -0.04498661])
    BOTTLE_GRASP_QUAT           = np.array([0.0, 0.0, -176])  # degrees, relative to bottle's orientation (yaw = 180 deg)
    BOTTLE_LIFT_UP_OFFSET       = np.array([0.0, 0.0, 0.04]) # Relative to BOTTLE_GRASP_POS
    # BOTTLE_PRE_POUR_OFFSET      = np.array([-0.11000000, -0.11000000, 0.12281656])    # w.r.t. the mug's frame
    BOTTLE_PRE_POUR_OFFSET      = np.array([-0.19000000, -0.18000000, 0.09500000])    # w.r.t. the mug's frame
