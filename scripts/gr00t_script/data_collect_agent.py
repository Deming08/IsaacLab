...
def main():
    ...
    while simulation_app.is_running() and successful_episodes_collected_count < MAX_EPISOIDES:
        with torch.inference_mode():
            ...
            rgb_image_np = obs["policy"]["rgb_image"].squeeze(0).cpu().numpy()
            depth_image_np = obs["policy"]["depth_image"].squeeze(0).cpu().numpy()
            segmentation_image_np = obs["policy"]["segmentation_image"].squeeze(0).cpu().numpy()
